...
| LEARN-REPLAY | Replay supports the selected word and recognized sentence/caption, preserves exact/estimated provenance, clamps exact word playback against adjacent boundaries, and restores playback state | `1546662`; next bounded interaction update | `test-popup-bindings.mjs`; deterministic interval and restoration coverage required | M-06 | Existing word replay source present; sentence replay and tighter clamping requested, not implemented |
...
| UI-STYLE | Transcript/translation have separate appearance, distinct default colors, a smaller translation default, and optional blur-until-reveal | `ea1379c`, `a2716b4`; next bounded interaction update | `test-learning-features.mjs`, `test-batch-lifecycle.mjs`; blur coverage required | M-08, U-02–U-03 | Independent settings and 31/21 px defaults present; blur requested, not implemented |
...
| ACQ-GENERIC | Clear MP4/HLS/DASH, same-origin loopback MP4, and embedded-player discovery remain available | `923d8c9`, versions `0.10.1`–`0.10.4` | `test-media-candidate.mjs`, `test-media-observer.mjs`, `test-network-media-observer.mjs`, `test-local-media.mjs`, `test-batch-lifecycle.mjs`, `test-browser-audio-fallback.mjs`, server URL/HLS/wrapper tests | M-14 | 2026-07-23 user-run success on current-version YouTube, AniWorld, and AnimeKai; 0.10.4 local-video browser check and detailed multi-sample M-14 evidence remain incomplete |
