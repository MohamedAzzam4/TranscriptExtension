...
| ACQ-GENERIC | Clear MP4/HLS/DASH and embedded-player discovery remain available | `923d8c9`, versions `0.10.1`–`0.10.3` | `test-media-candidate.mjs`, `test-media-observer.mjs`, `test-network-media-observer.mjs`, `test-batch-lifecycle.mjs`, `test-browser-audio-fallback.mjs`, server HLS/wrapper tests | M-14 | AniWorld user-run pass; AnimeKai PNG-wrapped MPEG-TS decoded in a bounded native probe, full Episode 5 browser run pending |
